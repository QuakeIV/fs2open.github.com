...
bool WaveFile::Open(const char* pszFilename, bool keep_ext) {
  ...
  try {
    ...
    m_audioStreamIndex = av_find_best_stream(ctx, AVMEDIA_TYPE_AUDIO, -1, -1, const_cast<const AVCodec**>(&audio_codec), 0);
    ...
    printf("SOUND ==> Could not open wave file %s for streaming. Reason: %s\n", filename, e.what());
    return false;
  }
